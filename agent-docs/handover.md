...
- The view-model extraction reduced `budgetExpenseManager.js` from 1,257 lines; later budget, Bank, and recurring-editor orchestration bring the current local file to 1,031 lines while presentation and form details remain in child bundles.
...
- The record-based Bank compatibility metadata, code, and same-org data migration are complete in `mainDevOrg`. They add global `Bank__c` records, `Expense_Group_Bank__c` assignments, optional Expense and Recurring Expense `Bank_Assignment__c` lookups, group-scoped selectors, and trigger validation. Both legacy Bank picklists and their global value set remain for fallback and rollback. Focused check-only deployment `0AfgK00000QusoLSAR` and deployment `0AfgK00000Qun3uSAB` each passed 51/51 components and all 58 tests with zero errors or coverage warnings; touched production coverage ranged from 92.96% to 100%. Migration created 5 active Banks and 15 active group assignments, then backfilled 483 Expenses and 8 Recurring Expenses; read-only verification found zero mismatches and unchanged record counts and amount totals. The one-off scripts were removed after verification, and the source is committed and pushed as `c3f7889`; signed-in Bank smoke testing remains pending.
- The follow-up after `c3f7889` adds the custom Recurring Add/Edit modal, fresh request-guarded Bank options, server date-window validation, capped-pointer replay protection, and ended-pointer filtering. Local formatting, lint, Code Analyzer, XML/inventory, and source-conversion checks pass. Check-only deployment `0AfgK00000R8yNfSAJ` and deployment `0AfgK00000R9RZKSA3` each compiled all 12 components and passed all 51 specified tests with zero errors or coverage warnings. The recurring schedule is restored as the only active job, `08egK00000cZuPiQAK`, in `WAITING` state at 08:00 Asia/Manila. Signed-in smoke testing, commit, and push remain pending; no Jest tests were added or run.
...
- The deleted settings row was exported with hashes and rollback identifiers before cleanup. The one-off local scripts and backup artifacts were later removed from this app repository at the user's request. The legacy custom object was not purged.
...
- Smoke test the deployed recurring editor: Add/Edit, active/inactive/legacy Bank handling, group switching, date validation, and catch-up generation.
