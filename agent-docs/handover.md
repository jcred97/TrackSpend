...
- Main branch is clean and pushed through `919e8a1 refactor: extract Spendly view models and expense transforms`.
- The current committed LWC source is deployed to `mainDevOrg`.
- Spendly uses `spendlyApp` as the workspace shell and state coordinator. Dashboard, Expenses, and Recurring presentation live in dedicated child components; pure derived data lives in feature-specific view-model modules.
- `spendlyApp.js` is 858 lines after the readability refactor, down from 1,257 lines before the view-model extraction.
...
- Keep data loading, mutable cross-view state, modal ownership, server calls, and action orchestration in `spendlyApp`. Presentation components communicate actions through events, while view-model modules remain pure and side-effect free.
...
- Resume the 451-finding SLDS hardcoded-value migration in focused, visually verified CSS slices.
- Start with one isolated component stylesheet at a time, run the focused analyzer, dry-run the affected bundle, deploy, and smoke-test in the Salesforce shell.
- Avoid splitting `spendlyApp` further unless a cohesive responsibility can be removed without scattering mutable state or Apex orchestration.
